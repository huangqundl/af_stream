...
typedef char zmq_msg_size_check
    [2 * ((sizeof (afs_zmq::msg_t) == sizeof (zmq_msg_t)) != 0) - 1];

bool afs_zmq::msg_t::check ()
{
     return u.base.type >= type_min && u.base.type <= type_max;
}

int afs_zmq::msg_t::init ()
{
    u.vsm.type = type_vsm;
    u.vsm.flags = 0;
    u.vsm.size = 0;
...
int afs_zmq::msg_t::init_size (size_t size_)
{
    if (size_ <= max_vsm_size) {
        ...
        if (afs_unlikely (!u.lmsg.content)) {
            errno = ENOMEM;
            return -1;
        }
        ...
        new (&u.lmsg.content->refcnt) afs_zmq::atomic_counter_t ();
    }
    return 0;
}

int afs_zmq::msg_t::init_data (void *data_, size_t size_, msg_free_fn *ffn_,
    void *hint_)
{
...
int afs_zmq::msg_t::init_delimiter ()
{
...
int afs_zmq::msg_t::close ()
{
    //  Check the validity of the message.
    if (afs_unlikely (!check ())) {
        errno = EFAULT;
        return -1;
    }
...
int afs_zmq::msg_t::move (msg_t &src_)
{
    //  Check the validity of the source.
    if (afs_unlikely (!src_.check ())) {
        errno = EFAULT;
        return -1;
    }
...
int afs_zmq::msg_t::copy (msg_t &src_)
{
    //  Check the validity of the source.
    if (afs_unlikely (!src_.check ())) {
        errno = EFAULT;
...
unsigned char afs_zmq::msg_t::flags ()
{
    return u.base.flags;
}

void afs_zmq::msg_t::set_flags (unsigned char flags_)
{
    u.base.flags |= flags_;
}

void afs_zmq::msg_t::reset_flags (unsigned char flags_)
{
    u.base.flags &= ~flags_;
}

bool afs_zmq::msg_t::is_identity () const
{
    return (u.base.flags & identity) == identity;
}

bool afs_zmq::msg_t::is_delimiter ()
{
    return u.base.type == type_delimiter;
}

bool afs_zmq::msg_t::is_vsm ()
{
    return u.base.type == type_vsm;
}

bool afs_zmq::msg_t::is_cmsg ()
{
    return u.base.type == type_cmsg;
}

void afs_zmq::msg_t::add_refs (int refs_)
{
    zmq_assert (refs_ >= 0);
...
bool afs_zmq::msg_t::rm_refs (int refs_)
{
    zmq_assert (refs_ >= 0);
