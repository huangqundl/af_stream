...
afs_zmq::poller_base_t::poller_base_t ()
{
}
...
int afs_zmq::poller_base_t::get_load ()
{
    return load.get ();
}

void afs_zmq::poller_base_t::adjust_load (int amount_)
{
    if (amount_ > 0)
        load.add (amount_);
    else
...
void afs_zmq::poller_base_t::add_timer (int timeout_, i_poll_events *sink_, int id_)
{
...
void afs_zmq::poller_base_t::cancel_timer (i_poll_events *sink_, int id_)
{
...
uint64_t afs_zmq::poller_base_t::execute_timers ()
{
    //  Fast track.
    if (timers.empty ())
        return 0;
