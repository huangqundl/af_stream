...
afs_zmq::v2_encoder_t::v2_encoder_t (size_t bufsize_,
        afs::OutCallbackBase* out_callback_) :
    encoder_base_t <v2_encoder_t> (bufsize_, out_callback_)
{
    //  Write 0 bytes to the batch and go to message_ready state.
    next_step (NULL, 0, &v2_encoder_t::message_ready, true);
}

afs_zmq::v2_encoder_t::~v2_encoder_t ()
{
}

void afs_zmq::v2_encoder_t::message_ready ()
{
    //  Encode flags.
...
void afs_zmq::v2_encoder_t::size_ready ()
{
    //  Write message body into the buffer.
    if (afs_unlikely(data==NULL)) {
