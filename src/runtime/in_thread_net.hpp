...
class InThreadNet : public InThread<InT, ROutT> {
...
public:
    ...
    InThreadNet(InCallbackBase* in_callback,
...
InThreadNet<InT, ROutT>::InThreadNet(InCallbackBase* in_callback,
            std::vector<OutCallbackBase*>* out_callbacks) :
    InThread<InT, ROutT>(),
...
template <class InT, class ROutT>
void InThreadNet<InT, ROutT>::AddOutQueue(ZeroRingBuffer<WInT>* q) {
    in_callback_->AddOutQueue(q);
}

template <class InT, class ROutT>
void InThreadNet<InT, ROutT>::AddSource(char* source) {
    listen_addr_ = source;
}

/*
template<class T>
void InThreadNet<T>::set_queue(afs::ZeroRingBuffer_new* q) {
}
*/

template <class InT, class ROutT>
void InThreadNet<InT, ROutT>::ThreadInitHandler() {
    ...
    zmq::tcp_listener_t *listener =
        new (std::nothrow) zmq::tcp_listener_t (
            in_callback_,
            out_callbacks_,
            *options);
...
template <class InT, class ROutT>
void InThreadNet<InT, ROutT>::ThreadMainHandler() {
...
template <class InT, class ROutT>
void InThreadNet<InT, ROutT>::ThreadFinishHandler() {
    /*
...
template <class InT, class ROutT>
int InThreadNet<InT, ROutT>::parse_uri (const char *uri_,
...
// Copied and modified from socket_base.cpp in 0MQ
template <class InT, class ROutT>
int InThreadNet<InT, ROutT>::check_protocol (const std::string &protocol_)
{
