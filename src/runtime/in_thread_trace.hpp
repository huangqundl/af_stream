 ...
 */
template <class InT>
class InThreadTrace : public InThread<InT, NullClass> {

typedef WrapItem<InT> WInT;

public:
    InThreadTrace(RouterBase* router);
...
template<class InT>
InThreadTrace<InT>::InThreadTrace(
        RouterBase* router
        ) :
...
template<class T>
void InThreadTrace<T>::AddOutQueue(ZeroRingBuffer<WInT>* q) {
    queues_.push_back(q);
}

template<class T>
void InThreadTrace<T>::ThreadInitHandler() {
...
template<class T>
void InThreadTrace<T>::ThreadMainHandler() {
    LOG_MSG("InThreadTrace (%d) start to run\n", getpid());
...
template<class T>
void InThreadTrace<T>::ThreadFinishHandler() {
