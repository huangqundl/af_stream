...
#ifndef __AFS_COUNTER_ARRAY_L1_HPP_INCLUDED__
#define __AFS_COUNTER_ARRAY_L1_HPP_INCLUDED__
...
template<class V>
class NumericArray_L1 : public FTOperator {
...
public:
    NumericArray_L1(size_t l, double d = 0);
    ~NumericArray_L1();
...
template<class V>
NumericArray_L1<V>::NumericArray_L1(size_t l, double d) : FTOperator() {
...
template<class V>
NumericArray_L1<V>::~NumericArray_L1() {
    free(array);
    free(backup_array);
}

template<class V>
void NumericArray_L1<V>::UpdateValue(unsigned int index, V value) {
    V old_diff = abs(array[index] - backup_array[index]);
    V new_diff = abs(value -  backup_array[index]);
    tot_diff = tot_diff - old_diff + new_diff;
    array[index] = value;
}

template<class V>
V NumericArray_L1<V>::GetValue(unsigned int index) {
    return array[index];
}

template<class V>
void NumericArray_L1<V>::Clear() {
...
template<class V>
double NumericArray_L1<V>::CalculateDivergence() {
    return tot_diff;
}
    
template<class V>
void NumericArray_L1<V>::SerializeState(BackupData &backup_data) {
    unsigned int max_diff_index = 0;
    for (unsigned int i=0; i<length; i++) {
        if (abs(array[i]-backup_array[i])>abs(array[max_diff_index]-backup_array[max_diff_index])) {
            max_diff_index = i;
        }
    }
    ...
    tot_diff -= abs(array[max_diff_index]-backup_array[max_diff_index]);
    backup_array[max_diff_index] = array[max_diff_index];
}
    
template<class V>
void NumericArray_L1<V>::RecoveryState(BackupData& backup_data) {
