...
class ComputeThreadThread : public afs::ComputeThread<afs::RawItem, afs::RawItem, afs::RawItem, afs::NullClass> {
public:
    ComputeThreadThread(int num_out_queue, afs::RouterBase* r) :
        afs::ComputeThread<afs::RawItem, afs::RawItem, afs::RawItem, afs::NullClass>(num_out_queue, r) {}

typedef afs::WrapItem<afs::RawItem> WRawItem;
...
private:
    ...
    unordered_map<size_t, double> updates;
    ...
    void ComputeThreadInit() {
        ...
        sync_thresh = config->getdouble("logistic_regression.sync_thresh", -1);
        afs_assert(sync_thresh!=-1, "sync_thresh is not set\n");
    ...
    void ProcessRecord(afs::RawItem& tuple, uint64_t seq) {
        ...
        //if (1) {
            ...
            if (n % 100 ==0) {
                ...
                compute_diff(weights->get_entire(), old_weights);
                send_updates();
...
void compute_diff(unordered_map<size_t,double>& new_w, unordered_map<size_t,double>& old_w){
    for(auto it = new_w.begin(); it != new_w.end(); it++) {
        size_t index = it->first;
        double minus = new_w[it->first] - old_w[it->first];
        if (abs(minus) > sync_thresh) {
            updates[index] = minus;
        }
    }
}
...
void send_updates() {
    WRawItem* witem = GetSlot();
    int dest = GetDest(witem->data());
    char* data = witem->data().raw_data;
    for(auto it = updates.begin(); it != updates.end(); it++) {
        size_t index = it->first;
        double v = it->second;
        char tmp[50];
        sprintf(tmp, "%lu:%.2lf ", index, v);
        int len = strlen(tmp);
        memcpy(data, tmp, len);
        data = data + len;
    }
    CompleteWriteSlot(dest, witem);
    updates.clear();
}
