#ifndef __PacketTokenGrouping_hpp__
#define __PacketTokenGrouping_hpp__
...
class PacketTokenGrouping : public LoadBalancer <Packet::BasicField> {
private:
    typedef struct {
        // key
...
public:
    PacketTokenGrouping() {
        LOG_MSG("LoadBalancer in ZMQMainAnalyzer: PacketTokenGrouping\n");
 ...
 ****************************************************************************/
unsigned int 
PacketTokenGrouping::flow_hash_fcn(const HASH_FLOW* f) {
    char key[7];
...
bool 
PacketTokenGrouping::flow_cmp_fcn(const HASH_FLOW* f1, const HASH_FLOW* f2) {
...
void 
PacketTokenGrouping::flow_init_fcn(HASH_FLOW* f, void* st_arg) {
    memset(f, 0, sizeof(HASH_FLOW));
}

void 
PacketTokenGrouping::flow_free_fcn(HASH_FLOW* f, void* st_arg) {
    memset(f, 0, sizeof(HASH_FLOW));
}

void 
PacketTokenGrouping::flow_travel_do_fcn(HASH_FLOW* f, void* context) {
}
