...
private:
    ...
    void ComputeThreadInit() {
        ...
        max_train = config->get_ull("adapter.max_data", 0);
    ...
    void ProcessRecord(afs::RawItem& tuple, uint64_t seq) {
        ...
        //if (1) {
            ...
            fprintf(output_convergence, "seq %lu result: %d predicted: %.02lf, average predict_err: %.03lf\n", seq, label, predicted, predict_err/n);
            if (n % 10000 == 0) {
