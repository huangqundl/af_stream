...
#include "Packet.hpp"
#include "router/RawPacketBalancer.hpp"
...
int main(int argc, char* argv[]) {
    ...
    // create worker
    Worker<RawPacket, Packet::BasicField, Dummy>* worker =
        new Worker<RawPacket, Packet::BasicField, Dummy>(argv[2], zk_server);

    // add collector
    ResultWriter mainComputeThread("BM1", num_compute_thread);
    worker->addCollector("BM1", &mainComputeThread);
	
    // add compute_threads
    PcapDecoder** subComputeThreads = new PcapDecoder*[num_compute_thread];
    for (int i = 0; i < num_compute_thread; i++) {
        subComputeThreads[i] = new PcapDecoder("BS", i);
