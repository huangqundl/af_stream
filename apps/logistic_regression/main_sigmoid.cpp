...
int main(int argc, char* argv[]) {
    ...
    // create worker
    Worker<Item, Dummy, Dummy>* worker =
        new Worker<Item, Dummy, Dummy>(argv[2], zk_server);

    // add collector
    DetectCollector collector("BM1", num_compute_thread);
    worker->addCollector("BM1", &collector);
	
    // add compute_threads
    RollComputeThread** compute_threads = new RollComputeThread*[num_compute_thread];
    for (int i = 0; i < num_compute_thread; i++) {
        compute_threads[i] = new RollComputeThread("BS", i, config);
