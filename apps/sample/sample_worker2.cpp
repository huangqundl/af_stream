...
#include "sample_compute_thread2.hpp"
...
int main(int argc, char* argv[]) {
    ...
    afs::Worker<DummyType, DummyType>* worker = 
        new afs::Worker<DummyType, DummyType>(std::string(worker_name), worker_id, std::string(""), num_compute_thread);
    ...
    afs::UpThreadNet<DummyType>* i_thread =
        new afs::UpThreadNet<DummyType>(num_in, num_compute_thread);
    ...
    for (int i=0; i<num_compute_thread; i++) {
        ...
        ComputeThreadThread2* compute_thread =
            new ComputeThreadThread2(num_downstream, router_tmp);
