...
zmq::session_base_t::~session_base_t ()
{
    ...
    fprintf(stderr, "session deconstruct\n");
...
void zmq::session_base_t::read_activated (pipe_t *pipe_)
{
    ...
    //fprintf(stderr, "session read active\n");
    ...
    if (likely (pipe_ == pipe)) {
        //fprintf(stderr, "to restart output\n");
...
{
    //fprintf(stderr, "session write active\n");
    // Skip activating if we're detaching this pipe
    if (pipe != pipe_) {
        zmq_assert (terminating_pipes.count (pipe_) == 1);
        return;
...
{
    fprintf(stderr, "session process term\n");
    zmq_assert (!pending);
    ...
    if (!pipe && !zap_pipe) {
        fprintf(stderr, "    proceed with term\n");
        proceed_with_term ();
        return;
    }
    ...
    if (pipe != NULL) {
        fprintf(stderr, "    term pipe\n");
        //  If there's finite linger value, delay the termination.
        //  If linger is infinite (negative) we don't even have to set
        //  the timer.
        if (linger_ > 0) {
            fprintf(stderr, "    linger %d\n", linger_);
