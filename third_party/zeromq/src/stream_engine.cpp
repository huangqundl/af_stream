...
{
    //fprintf(stderr, "stream_engine unplug\n");
    zmq_assert (plugged);
    plugged = false;
...
{
    //fprintf(stderr, "stream_engine terminate\n");
    unplug ();
    delete this;
}
...
{
    zmq_assert (mechanism != NULL);
    //fprintf(stderr, "zap msg available\n");
...
int zmq::stream_engine_t::push_one_then_decode_and_push (msg_t *msg_)
{
    /*
    if (*((int*)(msg_->data()))) {
        //fprintf(stderr, "switch decode finish\n");
    }
    */
...
{
    //fprintf(stderr, "stream_engine error\n");
    zmq_assert (session);
